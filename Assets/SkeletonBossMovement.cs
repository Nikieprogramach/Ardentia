...
public class SkeletonBossMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (playerInRadius)
        {
            ...
            transform.GetComponent<SkeletonBossAttack>().isEngaged = true;
            target = Physics2D.OverlapCircle(transform.position, 100, player).GetComponent<Transform>();
        }

        if (isFollowingPlayer == true && Vector2.Distance(transform.GetComponent<SkeletonBossAttack>().attackPoint.position, target.position) > transform.GetComponent<SkeletonBossAttack>().attackRange)
        {
